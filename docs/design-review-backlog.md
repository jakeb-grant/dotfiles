...
**Session 10b (2026-07-02) — regression fix: launcher (Super+Space) dead since QS-8.** Owner report: launcher stopped opening. Root cause was session 9's QS-8 unification — `Launcher.activeScreen = Hyprland.focusedMonitor?.screen` compared by **object identity** in Drawers, but a live probe proved `HyprlandMonitor.screen` returns a *distinct wrapper object* from the `Quickshell.screens` entry for the same physical screen (`monitorFor(screens[0]).screen === Quickshell.screens[0]` → **false**), so the comparison never matched and the panel never rendered. (Session 9's review note "ShellScreen identity comparison verified" was wrong — Popout's convention works because its instances come from the bar window's `.screen`, which *are* `Quickshell.screens` entries; the launcher was the only `focusedMonitor` consumer.) Fix: resolve `activeScreen` by monitor *name* to the actual `Quickshell.screens` instance, with a first-screen fallback for the probe-confirmed `focusedMonitor == null` startup window. Verified end-to-end through the real global-shortcut dispatch with debug instrumentation on the deployed files (SHORTCUT-FIRED → resolved eDP-1 instance) and a screenshot of the open launcher; instrumentation then stripped by re-apply, reload clean. Debugging detour worth remembering: one hot-reload silently *failed* on the known scanner race mid-test, leaving stale code running — always confirm "Configuration Loaded" follows the last file write before trusting a live test. Drive-bys: live Zed settings had drifted (`cli_default_open_behavior` added by Zed's UI — folded into the template before an apply could clobber it); `settings.json.tmpl` → `private_settings.json.tmpl` (Zed rewrites the file 0600; target now matches, killing a perpetual mode-diff).
