...
- [ ] Remaining conversions: `gtk-3.0/gtk.css`, `gtk-4.0/gtk.css`, `phylax/style.css`, `zed/settings.json`, obsidian `theme.css` (non-standard output path via `TEMPLATE_OUTPUT_OVERRIDES` — needs a decision: chezmoi `exact_`/external target or keep on a slim render step). Needs remaining filter helpers: `rgba`/`rgb`/`rgb_values` (hex→decimal digit-map), `hex_alpha`, `strip`; zed also uses `_zed_theme_dark`/`_light`/`_variant` meta keys (plain `$p._variant` etc.).
- [ ] Then: delete `process_templates` + the Jinja env + the jinja2 dependency from theme-switch; update CLAUDE.md/README to the final single-pipeline story.
