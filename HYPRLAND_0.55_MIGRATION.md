...
- [x] **Smoke-test legacy strings first** — confirmed they **do not** work in 0.55. `hyprctl dispatch killactive` errors with `hl.dispatch: expected a dispatcher`. All call sites rewritten.
...
- [x] `dot_config/quickshell/modules/bar/popouts/PowerPopout.qml`:53 — switch `hyprctl dispatch exit` to `hyprshutdown` (reference §5 explicitly prefers this).
- [x] `dot_config/quickshell/modules/bar/components/Workspaces.qml`:292 — `Services.Hypr.dispatch("workspace " + id)` → `Services.Hypr.dispatch('hl.dsp.focus({ workspace = ' + id + ' })')` if alias breaks.
